...
import { ArrowLeft, Calendar, Users, CheckCircle2, TrendingUp, Layers } from 'lucide-react';
  ...
  <ul className="space-y-3">
    {items.map((item, i) => (
      <li key={i} className="flex items-start gap-3 text-text-tertiary text-sm leading-relaxed">
        <span className="text-accent font-bold mt-0.5 flex-shrink-0 text-xs">{marker}</span>
        <span>{item}</span>
      </li>
    ))}
  </ul>
    ...
    <div className="min-h-screen bg-bg-primary pt-28 pb-24 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="max-w-5xl mx-auto">
              ...
              {project.highlights && (
                <DetailSection icon={CheckCircle2} title="Key Highlights" delay={0}>
                  <div className="rounded-2xl glass-card p-6">
                    <BulletList items={project.highlights} marker="✓" />
                  </div>
                </DetailSection>
              )}

              {project.impact && (
                <DetailSection icon={TrendingUp} title="Project Impact" delay={80}>
                  <div className="rounded-2xl glass-card p-6">
                    <BulletList items={project.impact} marker="▹" />
                  </div>
                </DetailSection>
              )}

              {project.keyFeatures && (
                <DetailSection icon={Layers} title="Key Features" delay={160}>
                  <div className="rounded-2xl glass-card p-6">
                    <BulletList items={project.keyFeatures} marker="→" />
                  </div>
                </DetailSection>
              )}
            </div>
            ...
            <div className="space-y-5">
              ...
              <div className="rounded-2xl glass-card p-5 sticky top-28">
                ...
                <div className="mt-5 pt-5 border-t border-border/40 space-y-3">
                  {project.client && (
                    <div>
                      <p className="text-[11px] text-text-quaternary uppercase tracking-wider mb-1">Client</p>
                      <p className="text-text-secondary text-sm font-medium">{project.client}</p>
                    </div>
                  )}
                  {project.year && (
                    <div>
                      <p className="text-[11px] text-text-quaternary uppercase tracking-wider mb-1">Year</p>
                      <p className="text-text-secondary text-sm font-medium">{project.year}</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          {/* ── CTA ── */}
          <div className="relative overflow-hidden rounded-3xl glass-card p-9 text-center mt-4">
