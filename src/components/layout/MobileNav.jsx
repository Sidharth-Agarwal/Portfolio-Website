...
const tabs = [
  ...
  { icon: Briefcase,label: 'Work',       section: 'experience',  path: '/'         },
  { icon: Code2,    label: 'Projects',   section: 'projects',    path: '/'         },
...
const MobileNav = () => {
  ...
  const handleTap = (tab) => {
    setActive(tab.label.toLowerCase());
    if (tab.section === null) {
      navigate('/');
    ...
    if (location.pathname !== tab.path) {
      navigate(tab.path);
      setTimeout(() => scrollTo(tab.section), 150);
    } else {
  ...
  return (
    ...
    <nav
      className="fixed bottom-0 inset-x-0 z-50 md:hidden
                 pb-safe"   /* respects iOS safe-area-inset-bottom */
